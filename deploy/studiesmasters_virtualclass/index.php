...
function call_backend($backendUrl, $secret, $username, $email, $action, $sessionId) {
    ...
    if ((isset($res['success']) && !$res['success'])
        ...
        // "unknown_user" means Moodle could not resolve this Moodle account to a
        // StudiesMasters principal (no MoodleLink row). It is an ACCOUNT-LINKING
        // problem, not "you have no classes" — the generic message made it look
        // like an empty timetable and sent admins hunting for missing classes.
        if ($msg === 'unknown_user' || (isset($res['reason']) && $res['reason'] === 'unknown_user')) {
            $msg = 'Your Moodle account is not linked to a StudiesMasters account yet, '
                 . 'so your classes cannot be loaded. Please contact your administrator '
                 . 'to have your account linked — quoting Moodle username "' . $GLOBALS['USER']->username . '".';
        }
